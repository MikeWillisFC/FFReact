...
const FCReactDesignToolPrompt = props => {
   ...
   let handleClick = event => {
	...
	const ref_isFirstRender = useRef(true);

	// only run on first render, see https://stackoverflow.com/a/76242764/1042398
	useEffect(() => {
		if ( ref_isFirstRender.current ) {
		...
		ref_isFirstRender.current = false;
		return () => { ref_isFirstRender.current = true };
	}, []);
